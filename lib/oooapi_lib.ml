...
let endpoint =
  [%str
    ...
    module EndpointLib (Server : Server) (Config : Config) = struct
      ...
      type form_part = string * [ `String of string | `File of string ]

      type data =
        [ `Json of Yojson.Safe.t
        | `Multipart_form of form_part list
        ]
      ...
      let make_request
        ...
        in
        let req_headers = Cohttp.Header.add_list default_headers headers in
